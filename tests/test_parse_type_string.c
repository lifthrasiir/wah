...
int main() {
    ...
    printf("Test 1: i32 -> i32\n");
    {
        ...
        assert_ok(wah_debug_parse_func_spec(types, &nparams, &param_types, &nresults, &result_types));
    ...
    printf("Test 7: Invalid types should fail\n");
    {
        ...
        for (size_t i = 0; i < sizeof(invalid_types) / sizeof(invalid_types[0]); i++) {
            ...
            printf("  %s\n", types);
            assert_err(wah_debug_parse_func_spec(types, &nparams, &param_types, &nresults, &result_types), WAH_ERROR_BAD_SPEC);
            free(param_types);
            free(result_types);
        }
    }
    ...
    printf("Test: Invalid type spec character\n");
    {
        size_t nparams, nresults;
        wah_type_t *param_types = NULL, *result_types = NULL;
        assert_err(wah_debug_parse_func_spec("x32 -> i32", &nparams, &param_types, &nresults, &result_types), WAH_ERROR_BAD_SPEC);
        free(param_types);
        free(result_types);
        param_types = NULL; result_types = NULL;
        assert_err(wah_debug_parse_func_spec("i32 -> z64", &nparams, &param_types, &nresults, &result_types), WAH_ERROR_BAD_SPEC);
        free(param_types);
        free(result_types);
    }
