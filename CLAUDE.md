...
- `wah.h` (~17,000 lines): The entire interpreter. Lines 1-1345 are the public API (before `#ifdef WAH_IMPLEMENTATION` on line 1345). Everything after is implementation. The public portion is C++11-compatible. `WAH_VERSION` exposes the API version.
...
4. **Instantiation:** `wah_instantiate()` resolves imports from linked modules (including re-exported imports that chain through multiple modules up to `WAH_REEXPORT_MAX_DEPTH`), initializes memories/tables/globals, evaluates const expressions, runs element/data segment initialization in both primary and linked modules, and executes the start function for all modules. After instantiation, modules referenced by an exec context are treated as immutable; the exec context never mutates linked modules. The instantiation logic is decomposed into helper functions (e.g., `wah_resolve_primary_func_imports`, `wah_prepare_linked_globals`, `wah_propagate_table_import_aliases`, etc.).
...
- **GC struct/array specializations:** `struct.get`/`struct.set` and `array.get`/`array.set` are specialized at analyze time by field/element size into concrete opcodes (e.g., `STRUCT_GET_S8`, `STRUCT_SET_16`, `ARRAY_GET_U8`, `ARRAY_SET_64`), eliminating runtime type dispatch.
...
The GC uses a mark-sweep collector (`wah_gc_step`). Objects are allocated with `wah_gc_alloc()` which prepends a `wah_gc_object_t` header containing a tagged next pointer (low 2 bits encode mark/aux flags); allocation sizes are checked for header overflow. `wah_repr_info_t` describes struct/array layouts with field offsets and a reference map for tracing; struct layout is computed by a padding-minimizing builder. i31ref values use pointer tagging (low bit = 1). All references except i31ref (including funcrefs via `wah_function_holder_t`) carry a GC header before their payload. Exception objects are also GC-managed (`wah_gc_alloc_exception`), with variable-sized payloads storing values and types. `wah_gc_alloc_host` allocates externrefs/anyrefs from the host side; `wah_gc_heap_stats[_from_host]` and `wah_gc_verify_heap` expose introspection. Cross-module GC is supported: dependent exec contexts (tracked via `dependents` arrays and `wah_gc_state_t.gc_dependents`) are scanned during collection so that objects reachable from linked modules' globals and tables are not prematurely freed.
...
When a WASM function calls into a linked module, the call frame stores `frame->module`, `frame->frame_globals`, `frame->frame_function_table`, and `frame->frame_ctx` to track the callee's module context. The `fctx` variable in the interpreter is the "active" exec context for memory/table access, swapped on cross-module transitions. `table.grow` and `memory.grow` propagate aliased pointer updates to all dependent contexts to avoid stale pointers. Cross-module subtype checks use `wah_cross_module_subtype()` with a shared `type_check_cache[]`.
...
- **Linking API:** Use `wah_link_module()` to link a parsed/built module by import name, or `wah_link_context()` to link an already-instantiated exec context. Linked contexts are tracked as dependents for grow-alias propagation and cross-module GC.
- **Import Resolution:** During `wah_instantiate()`, the interpreter resolves imports by matching import declarations to exports from linked modules. Re-exported imports are resolved transitively (up to `WAH_REEXPORT_MAX_DEPTH = 64` hops). Failures return `WAH_ERROR_LINK_FAILED` (formerly `WAH_ERROR_IMPORT_NOT_FOUND`). Import type validation covers functions, globals, memories, tables, and tags across module boundaries.
...
All public API functions return a `wah_error_t` enum value. `WAH_OK` indicates success. Use `wah_strerror(err)` for human-readable error descriptions. Resumable status codes (`WAH_STATUS_FUEL_EXHAUSTED`, `WAH_STATUS_YIELDED`) are positive values distinct from error codes. `WAH_ERROR_MULTI_RETURN` (no longer the success-flavored `WAH_OK_BUT_MULTI_RETURN`) signals that a single-return entry point was used on a multi-return function. `WAH_ERROR_MISUSE` is returned for API misuse such as NULL arguments to public functions.
...
OOM and leak hygiene are tested explicitly (`tests/test_oom.c` injects allocation failures, `tests/test_misuse.c` validates error handling for invalid API usage, and the codebase is run under ASAN/UBSAN/LSAN — see Makefile flags).
...
- **Platform-specific SIMD:** x86-64 uses SSE2 baseline with runtime detection for SSSE3/SSE4.1/SSE4.2/AVX2/AVX-512 via CPUID (32-bit x86 is not supported). AArch64 uses NEON. Inline asm wrappers (`WAH_ASM_*` macros) allow using higher ISA instructions without `-march` flags. Generic fallbacks exist for all SIMD ops. On little-endian x86-64/AArch64, `WAH_NATIVE_LE` enables memcpy-based LE loads/stores that compile to single machine instructions.
...
- **Exception Handling:** Uses `try_table` with a fixed-depth exception handler stack (`WAH_MAX_EXCEPTION_HANDLER_DEPTH = 64`). `wah_exception_t` is a GC-managed object (allocated via `wah_gc_alloc_exception`) carrying a GC header, tag identity, tag index, value count, and variable-sized payload (values followed by types). `throw` sets `pending_exception` and unwinds the call stack looking for matching handlers.

- **Host Function Integration:** `wah_export_func()` accepts a function type string DSL (e.g., `"(i32, i32) -> i32"`). Host functions receive a `wah_call_context_t` with typed param/result accessors (`wah_param_*`, `wah_result_*`, `wah_param_type`, `wah_result_type`, `wah_param_count`, `wah_result_count`). Return values are truncated to declared types just like wasm-side returns. Use `wah_trap` to abort with a specific error. GC references in params are valid only for the call duration; host must not retain pointers. `wah_param_ref` is restricted from returning non-host pointers; externrefs in `wah_call*` return values are sanitized.
