...
private:
    ...
    struct wah_function_holder_s *functions; // unified function array
...
#define WAH_I64_OTHER_OPCODES(X) \
    WAH_IF_MEMORY64( \
...
#define WAH_LOWER_MEM_CASE(name, suffix) case WAH_OP_##name: opcode = WAH_OP_##name##_##suffix; break;
            ...
            default: switch (opcode) {
                ...
                #define WAH_XLAT_OP(new_op, ft, base_op) uint16_t new_op; switch (ft) { \
                    case WAH_TYPE_PACKED_I8: new_op = WAH_OP_##base_op##_8; break; \
                    case WAH_TYPE_PACKED_I16: new_op = WAH_OP_##base_op##_16; break; \
                    case WAH_TYPE_I32: case WAH_TYPE_F32: new_op = WAH_OP_##base_op##_32; break; \
                    case WAH_TYPE_I64: case WAH_TYPE_F64: new_op = WAH_OP_##base_op##_64; break; \
                    case WAH_TYPE_V128: new_op = WAH_OP_##base_op##_128; break; \
                    default: new_op = (sizeof(void *) >= 8) ? WAH_OP_##base_op##_64 : WAH_OP_##base_op##_32; break; \
                }
                case WAH_OP_STRUCT_GET: case WAH_OP_STRUCT_GET_S: case WAH_OP_STRUCT_GET_U: {
                    ...
                    #define WAH_OP_STRUCT_GET_8 (opcode == WAH_OP_STRUCT_GET_S ? WAH_OP_STRUCT_GET_S8 : WAH_OP_STRUCT_GET_U8)
                    #define WAH_OP_STRUCT_GET_16 (opcode == WAH_OP_STRUCT_GET_S ? WAH_OP_STRUCT_GET_S16 : WAH_OP_STRUCT_GET_U16)
                    WAH_XLAT_OP(new_op, ft, STRUCT_GET);
                    #undef WAH_OP_STRUCT_GET_8
                    #undef WAH_OP_STRUCT_GET_16
                ...
                case WAH_OP_STRUCT_SET: {
                    ...
                    wah_type_t ft = module->type_defs[typeidx].field_types[fieldidx];
                    const wah_repr_info_t *ri = module->repr_infos[module->typeidx_to_repr[typeidx]];
                    WAH_XLAT_OP(new_op, ft, STRUCT_SET);
                    wah_write_u16_le(buf + buf_size - sizeof(uint16_t), new_op);
                    WAH_LOWER_U32(ri->fields[fieldidx].offset);
                    break;
                }
                case WAH_OP_ARRAY_GET: case WAH_OP_ARRAY_GET_S: case WAH_OP_ARRAY_GET_U: {
                    ...
                    #define WAH_OP_ARRAY_GET_8 (opcode == WAH_OP_ARRAY_GET_S ? WAH_OP_ARRAY_GET_S8 : WAH_OP_ARRAY_GET_U8)
                    #define WAH_OP_ARRAY_GET_16 (opcode == WAH_OP_ARRAY_GET_S ? WAH_OP_ARRAY_GET_S16 : WAH_OP_ARRAY_GET_U16)
                    WAH_XLAT_OP(new_op, et, ARRAY_GET);
                    #undef WAH_OP_ARRAY_GET_8
                    #undef WAH_OP_ARRAY_GET_16
                    wah_write_u16_le(buf + buf_size - sizeof(uint16_t), new_op);
                    break;
                }
                case WAH_OP_ARRAY_SET: {
                    uint32_t typeidx = instr->imm.u32;
                    wah_type_t et = module->type_defs[typeidx].field_types[0];
                    WAH_XLAT_OP(new_op, et, ARRAY_SET);
                    wah_write_u16_le(buf + buf_size - sizeof(uint16_t), new_op);
                    break;
                }
                #undef WAH_XLAT_OP
...
static bool wah_find_linked_module(
    const wah_exec_context_t *ctx, const wah_import_name_t *name,
    const wah_module_t **out_module, wah_exec_context_t **out_linked_ctx, uint32_t *out_linked_idx
) {
    WAH_ASSERT(ctx);
    WAH_ASSERT(name);
...
static wah_error_t wah_validate_function_import_type(
    ...
    WAH_ENSURE(import_type_idx < importer->type_count, WAH_ERROR_LINK_FAILED);
    if (src->is_host) {
        const wah_func_type_t *import_type = &importer->types[import_type_idx];
...
static wah_error_t wah_bind_memory_import_slot(
    ...
    WAH_ENSURE(provider_ctx && mem_idx < provider_ctx->memory_count, WAH_ERROR_LINK_FAILED);
    WAH_ENSURE(mem_idx < wah_memory_index_limit(provider), WAH_ERROR_LINK_FAILED);
    const wah_memory_type_t *provider_type = wah_memory_type(provider, mem_idx);
    WAH_ENSURE(provider_type->addr_type == import_type->addr_type, WAH_ERROR_LINK_FAILED);
    if (import_type->max_pages != UINT64_MAX) {
    ...
    *module = (wah_module_t){0}; // Initialize module struct
    ...
    if (module->wasm_function_count > 0) {
        ...
        for (uint32_t i = 0; i < module->wasm_function_count; i++) {
            module->functions[i] = (wah_function_holder_t){
                .header = (wah_gc_object_t)WAH_FUNCREF_HEADER,
                .func = { .local_idx = i, .global_idx = module->import_function_count + i, .fn_module = module },
            };
        }
...
static void wah_gc_visit_module_globals(const wah_module_t *module, wah_value_t *globals,
                                        wah_gc_ref_visitor_t visitor, void *userdata) {
    for (uint32_t i = 0; i < wah_global_index_limit(module); i++) {
...
static void wah_gc_enumerate_roots(wah_exec_context_t *ctx, wah_gc_ref_visitor_t visitor, void *userdata) {
    ...
    wah_gc_visit_module_globals(module, ctx->globals, visitor, userdata);
    ...
    *exec_ctx = (wah_exec_context_t){ .is_instantiated = false, .alloc = wah_resolve_alloc(options ? options->alloc : NULL) };
    ...
    if (table_size > 0) {
        ...
        for (uint32_t i = 0; i < import_count; i++) {
            exec_ctx->function_table[i] = (wah_function_holder_t){ .header = (wah_gc_object_t)WAH_FUNCREF_HEADER };
        }
...
static inline bool wah_ref_test_heap_type(wah_exec_context_t *ctx, wah_value_t ref_val, wah_type_t target) {
    ...
    switch (ht) {
        ...
        case WAH_TYPE_ANY: case WAH_TYPE_EXTERN: return repr_id != WAH_TYPE_FUNC;
...
static wah_function_t *wah_materialize_funcref(wah_exec_context_t *fctx, uint32_t func_idx) {
    WAH_ASSERT(func_idx < fctx->function_table_count);
    wah_function_t *fn = &fctx->function_table[func_idx].func;
...
static void wah_update_table_import_aliases(wah_exec_context_t *target, uint32_t exclude_idx,
                                            wah_exec_context_t *owner_ctx, uint32_t owner_idx,
                                            wah_value_t *entries, uint64_t size) {
    for (uint32_t j = 0; j < target->table_count; j++) {
        if (j == exclude_idx) continue;
        if (target->tables[j].is_imported &&
            target->tables[j].import_ctx == owner_ctx &&
            target->tables[j].import_idx == owner_idx) {
            target->tables[j].entries = entries;
            target->tables[j].size = size;
        }
    }
}

static void wah_update_memory_import_aliases(wah_exec_context_t *target, uint32_t exclude_idx,
                                             wah_exec_context_t *owner_ctx, uint32_t owner_idx,
                                             uint8_t *data, uint64_t size) {
    for (uint32_t j = 0; j < target->memory_count; j++) {
        if (j == exclude_idx) continue;
        if (target->memories[j].is_imported &&
            target->memories[j].import_ctx == owner_ctx &&
            target->memories[j].import_idx == owner_idx) {
            target->memories[j].data = data;
            target->memories[j].size = size;
            if (j == 0) {
                target->memory_base = target->memories[0].data;
                target->memory_size = target->memories[0].size;
            }
        }
    }
}

static void wah_propagate_table_import_aliases(wah_exec_context_t *ctx, wah_exec_context_t *fctx,
                                               wah_exec_context_t *owner_ctx, uint32_t owner_idx,
                                               wah_value_t *entries, uint64_t size) {
    if (ctx != fctx) {
        wah_update_table_import_aliases(ctx, UINT32_MAX, owner_ctx, owner_idx, entries, size);
    }
    for (uint32_t m = 0; m < ctx->linked_module_count; m++) {
        wah_exec_context_t *lctx = ctx->linked_modules[m].ctx;
...
static void wah_propagate_memory_import_aliases(wah_exec_context_t *ctx, wah_exec_context_t *fctx,
                                                ...
                                                uint8_t *data, uint64_t size) {
    if (ctx != fctx) {
        wah_update_memory_import_aliases(ctx, UINT32_MAX, owner_ctx, owner_idx, data, size);
    }
    for (uint32_t m = 0; m < ctx->linked_module_count; m++) {
        wah_exec_context_t *lctx = ctx->linked_modules[m].ctx;
        if (!lctx || lctx == fctx) continue;
...
WAH_RUN(REF_FUNC) {
    ...
    (*sp++).ref = wah_func_to_ref(&frame->frame_function_table[func_idx].func);
    WAH_NEXT();
}
...
WAH_RUN(CALL) {
    ...
    const wah_function_t *called_fn = &frame->frame_function_table[called_func_idx].func;
...
void wah_free_module(wah_module_t *module) {
    ...
    if (module->functions) {
        for (uint32_t i = module->wasm_function_count; i < module->local_function_count; ++i) {
            wah_function_t *fn = &module->functions[i].func;
            if (fn->is_host) {
...
static wah_error_t wah_module_register_host_func(
    ...
    mod->functions[new_func_idx] = (wah_function_holder_t){
        .header = (wah_gc_object_t)WAH_FUNCREF_HEADER,
        .func = { .is_host = true,
...
static wah_error_t wah_init_active_elem_segments(wah_exec_context_t *ctx) {
    ...
    for (uint32_t i = 0; i < module->element_segment_count; ++i) {
        ...
        WAH_ENSURE(wah_u64_range_in_bounds(offset, segment->num_elems,
                                           ctx->tables[segment->table_idx].size), WAH_ERROR_TRAP);
...
static wah_error_t wah_init_active_data_segments(wah_exec_context_t *ctx) {
    ...
    for (uint32_t i = 0; i < module->data_segment_count; ++i) {
        ...
        if (segment->flags == 0x00 || segment->flags == 0x02) {
            ...
            WAH_ENSURE(wah_u64_range_in_bounds(offset, segment->data_len,
                                               ctx->memories[segment->memory_idx].size), WAH_ERROR_MEMORY_OUT_OF_BOUNDS);
            if (segment->data_len > 0) {
...
static wah_error_t wah_prepare_linked_globals(wah_exec_context_t *ctx) {
    ...
    for (uint32_t j = 0; j < ctx->linked_module_count; j++) {
        ...
        WAH_ENSURE(total_globals <= UINT32_MAX - gc, WAH_ERROR_TOO_LARGE);
        total_globals += gc;
    }

    if (total_globals <= wah_global_index_limit(module)) return WAH_OK;

    wah_value_t *new_globals = NULL;
    WAH_MALLOC_ARRAY(new_globals, total_globals);
...
static wah_error_t wah_resolve_primary_func_imports(wah_exec_context_t *ctx) {
    ...
    for (uint32_t i = 0; i < module->import_function_count; i++) {
        ...
        WAH_ENSURE(wah_find_linked_module(ctx, &fi->name, &linked, &fi_linked_ctx, NULL), WAH_ERROR_LINK_FAILED);
...
static inline wah_value_t *wah_resolved_global_slot(wah_exec_context_t *owner_ctx, uint32_t global_idx,
                                                    bool imported_mutable) {
...
static wah_error_t wah_resolve_primary_global_imports(wah_exec_context_t *ctx) {
    ...
    for (uint32_t i = 0; i < module->import_global_count; i++) {
        ...
        WAH_ENSURE(wah_find_linked_module(ctx, &gi->name, &linked, &gi_linked_ctx, NULL), WAH_ERROR_LINK_FAILED);
        ...
        if (linked_global_idx < linked->import_global_count && gi_linked_ctx) {
            ...
            WAH_CHECK(wah_resolve_global_export(ctx, linked, NULL, linked_global_idx, &global_provider,
...
static wah_error_t wah_create_tag_contexts_for_linked_modules(wah_exec_context_t *ctx) {
    const wah_alloc_t *alloc = &ctx->alloc;
    const wah_module_t *module = ctx->module;
    uint32_t g_offset = wah_global_index_limit(module);
    for (uint32_t j = 0; j < ctx->linked_module_count; j++) {
...
static wah_error_t wah_resolve_primary_tag_imports(wah_exec_context_t *ctx) {
    const wah_module_t *module = ctx->module;
    for (uint32_t i = 0; i < module->import_tag_count; i++) {
        ...
        WAH_ENSURE(wah_find_linked_module(ctx, &tgi->name, &linked, &linked_ctx, NULL), WAH_ERROR_LINK_FAILED);

        const wah_export_t *exp = wah_find_export(linked, 4, &tgi->name);
        WAH_ENSURE(exp != NULL, WAH_ERROR_LINK_FAILED);

        uint32_t linked_tag_idx = exp->index;
        WAH_ENSURE(linked_tag_idx < linked->import_tag_count + linked->tag_count, WAH_ERROR_LINK_FAILED);
...
static void wah_fixup_linked_gc_contexts(wah_exec_context_t *ctx) {
    for (uint32_t j = 0; j < ctx->linked_module_count; j++) {
    ...
    for (uint32_t j = 0; j < ctx->linked_module_count; j++) {
        ...
        if (ctx->linked_modules[j].ctx == NULL || ctx->linked_modules[j].owns_ctx) {
            for (uint32_t gi_idx = 0; gi_idx < lmod->import_global_count; gi_idx++) {
                ...
                WAH_ENSURE(prov_gidx < wah_global_index_limit(provider), WAH_ERROR_LINK_FAILED);
                ...
                WAH_CHECK(wah_resolve_global_export(ctx, provider, provider_ctx, prov_gidx, &actual_provider,
...
static wah_error_t wah_import_existing_memory(wah_exec_context_t *ctx, uint32_t dst_idx,
                                              wah_exec_context_t *linked_ctx, uint32_t linked_mem_idx,
                                              uint64_t min_pages) {
...
static wah_error_t wah_resolve_primary_memory_imports(wah_exec_context_t *ctx) {
    const wah_module_t *module = ctx->module;
...
static wah_error_t wah_finalize_owned_linked_contexts(wah_exec_context_t *ctx) {
    ...
    for (uint32_t j = 0; j < ctx->linked_module_count; j++) {
        ...
        if (ictx && !ictx->dropped_elem_segments && lmod->element_segment_count > 0) {
            ...
            WAH_MALLOC_ARRAY(ictx->dropped_elem_segments, bytes);
        ...
        if (ictx && !ictx->dropped_data_segments && lmod->data_segment_count > 0) {
            ...
            WAH_MALLOC_ARRAY(ictx->dropped_data_segments, bytes);
...
uint32_t wah_module_type_count(const wah_module_t *module) { return module ? module->type_count : 0; }
uint32_t wah_module_function_count(const wah_module_t *module) { return module ? module->import_function_count +
                                                                                 module->local_function_count : 0; }
uint32_t wah_module_global_count(const wah_module_t *module) { return module ? wah_global_index_limit(module) : 0; }
uint32_t wah_module_memory_count(const wah_module_t *module) { return module ? wah_memory_index_limit(module) : 0; }
uint32_t wah_module_table_count(const wah_module_t *module) { return module ? wah_table_index_limit(module) : 0; }
uint32_t wah_module_tag_count(const wah_module_t *module) { return module ? module->import_tag_count + module->tag_count : 0; }
uint32_t wah_module_import_count(const wah_module_t *module) { return module ? module->import_count : 0; }
uint32_t wah_module_export_count(const wah_module_t *module) { return module ? module->export_count : 0; }
...
wah_error_t wah_module_function(const wah_module_t *module, uint32_t funcidx, wah_func_desc_t *out) {
    ...
    *out = (wah_func_desc_t){0};
    ...
    if (module->functions[local_fn_idx].func.is_host) {
        const wah_function_t *fn = &module->functions[local_fn_idx].func;
        *out = (wah_func_desc_t){
